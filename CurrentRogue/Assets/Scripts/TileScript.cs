...
public class TileScript : MonoBehaviour 
{
	...
	//for dmg
	public Dictionary <int, HealthScript> HScrDict = new Dictionary <int, HealthScript> ();
	...
	*/
	...
	private void SetTarget (string _objStr, int _gunID, int _shipID)
	{
		...
		//goddamn! ye little fuck! no wonder it doesnt work...

		ShipScript _ship = LevelManager.Instance.Ships [_shipID].GetComponent <ShipScript> (); //transform.parent.parent.GetComponent <ShipScript> ();
	...
	*/
	...
	public HealthScript [] GetHScripts (int rmDmg, int sysDmg, int subDmg) {
		...
		if (rmDmg == 0) {
			...
			_hScrArr [0] = HScrDict [0].GetOriginHScr ();
		}

		if (sysDmg == 0) {
			Debug.LogError ("Damage is 0!");
		} else {
			if (HScrDict.ContainsKey (1)) {
				//LocalObjDict [0].TakeCrewDamage (rmDmg);
				_hScrArr [1] = HScrDict [1].GetOriginHScr ();
			} else if (!SystemPlacable) {
				//not the nicest way of doin' it but...
				HScrDict.Add (1, transform.GetChild (1).GetChild (0).gameObject.GetComponent <HealthScript> ());
				Debug.Log ("dict entry made");
				//LocalObjDict [0].TakeCrewDamage (rmDmg);
		...
		if (subDmg == 0) {
			...
			if (HScrDict.ContainsKey (2)) {
				//LocalObjDict [0].TakeCrewDamage (rmDmg);
				_hScrArr [2] = HScrDict [2].GetOriginHScr ();
			} else if (transform.GetChild (6).childCount > 0) {
				//not the nicest way of doin' it but...
				HScrDict.Add (2, transform.GetChild (6).GetChild (0).gameObject.GetComponent <HealthScript> ());
				Debug.Log ("dict entry made");
				//LocalObjDict [0].TakeCrewDamage (rmDmg);
	...
	public void PlaceTarget (int _gunID, int _shipID) {
		SetTarget ("Target", _gunID, _shipID);
	}
